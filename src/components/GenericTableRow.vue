...
<template>
  <template :key="row?.id" v-for="(row, index) in rowData">
    ...
    <tr
      @click="emit('row', row)"
      class="cursor-pointer hover:bg-gray-100  border-x-[0.2px] border-b-[0.2px] border-t-[0.2px]"
    >
      <td v-if="firstCol" class="p-3">
        <slot name="select" :row="row" />
      </td>
      <td class="p-4 font-medium">{{ index + 1 }}</td>
      <td class="p-2 py-4 font-medium max-w-40" :key="key" v-for="key in rowKeys">
