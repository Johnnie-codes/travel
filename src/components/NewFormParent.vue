...
<template>
  <div
    :class="[$style[size]]"
    class="overflow-hidden px-2 flex flex-col justify-between gap-4 bg-white rounded-md"
  >
    <div
    ...
    >
      ...
      <div class="flex items-center gap-4">
        ...
        <!-- <button class="border rounded-full" @click="closeModal()">
          <BaseIcon :path="mdiClose" :size="20" />
        </button> -->
      </div>
    </div>
    <div
      class="px-2 flex flex-col gap-4 form-scrollbar flex-1 py-2 overflow-auto border-b border-text-secondary-clr/30"
    >
      <slot />
